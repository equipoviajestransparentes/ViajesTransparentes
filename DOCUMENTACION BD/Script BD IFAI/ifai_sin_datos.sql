...
CREATE TABLE IF NOT EXISTS `ifai`.`comision` (
  `idComision` INT NOT NULL AUTO_INCREMENT COMMENT 'Llave identificadora de las comisiones',
  ...
  `UR` VARCHAR(1000) NOT NULL COMMENT 'Campo que indica la unidad responsable en la que se genera la nota de viaje, el oficio relacionado a la comisión a la que se envía al funcionario público, u otra información específica sobre motivo del viaje y resultados',
  `CONSECUTIVO` VARCHAR(50) NOT NULL COMMENT 'Campo que indica el número de identificación único asignado por las áreas de control del IFAI para identificar la comisión',
  `ACUERDO` VARCHAR(1000) NOT NULL COMMENT 'Campo que indica el número de acuerdo o documento oficial mediante el cual se autoriza la comisión del servidor público',
  ...
  `MOTIVO` VARCHAR(1000) NOT NULL COMMENT 'Campo que indica el motivo por el cual el funcionario público fue enviado a la comisión en cuestión',
  `COMPROBADO_TOTAL` DECIMAL(12,2) NULL DEFAULT 0.0 COMMENT 'Campo que indica el monto comprobado total de la comision asignada al servidor publico',
  ...
  `RESULTADO` VARCHAR(1000) NULL COMMENT 'Campo que contiene los resultados obtenidos durante la comision',
  `OBSERVACIONES` VARCHAR(1000) NULL COMMENT 'Campo que contiene las observaciones realizadas durante la comision',
